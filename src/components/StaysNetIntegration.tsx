...
export default function StaysNetIntegration() {
  ...
  const loadConfig = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/settings/staysnet`,
        {
          headers: {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/staysnet/test`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleTestEndpoint = async (endpoint: ApiEndpoint) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/staysnet/test-endpoint`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleFetchReservations = async (startDate?: string, endDate?: string) => {
    ...
    try {
      ...
      let url = `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/staysnet/reservations/preview`;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Integração com PMS avançado • External API v1.0
            </p>
          </div>
        </div>
