...
export default function WhatsAppIntegration() {
  ...
  const webhookUrl = `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/chat/channels/whatsapp/webhook`;
      ...
      try {
        ...
        if (localConfig.whatsapp) {
          setWhatsappForm({
            ...
            instance_token: localConfig.whatsapp.instance_token || ''
          });
        }
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      if (error.message?.includes('401') || error.message?.includes('Unauthorized')) {
        ...
        toast.error('❌ Erro de conexão! O servidor não está acessível', {
          duration: 6000,
        });
      } else {
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      try {
        ...
        if (result.success) {
          ...
          if (isFallback) {
            toast.success('✅ Configurações salvas localmente!', {
              description: '🔄 Modo offline ativo. Deploy o backend para sincronizar.',
              duration: 6000,
            });
            toast.info('💡 Para deployar o backend, execute: bash DEPLOY_BACKEND_NOW.sh', {
              duration: 10000,
            });
          } else {
        ...
        // Backend não acessível - salvar localmente
        console.warn('⚠️ Backend não acessível, salvando localmente:', fetchError);
        ...
        console.log('💾 Configurações salvas localmente no navegador');
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl mt-1">
                      {config?.whatsapp?.connected ? 'Online' : 'Offline'}
                    </p>
                  </div>
                  <div className={`w-12 h-12 rounded-full ${config?.whatsapp?.connected ? 'bg-green-500/10' : 'bg-gray-500/10'} flex items-center justify-center`}>
                    {config?.whatsapp?.connected ? (
