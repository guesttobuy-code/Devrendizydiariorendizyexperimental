...
export function PricingSettingsForm({ listingId }: { listingId: string }) {
  ...
  const fetchSettings = async () => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/listings/${listingId}/pricing-settings`,
        {
          headers: {
  ...
  const calculatePreview = async () => {
    ...
    try {
      setCalculating(true);
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/calculate-reservation`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
