...
export function SettingsManager({
  ...
  const loadSettings = async () => {
    ...
    try {
      if (mode === 'global') {
        ...
        const response = await fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/listings/${listingId}/settings`,
          {
            headers: { 'Authorization': `Bearer ${publicAnonKey}` }
          }
        );
  ...
  const resetToGlobal = async () => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/listings/${listingId}/settings/reset`,
        {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${publicAnonKey}` }
        }
  ...
  const toggleOverride = async (section: string, enabled: boolean) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/listings/${listingId}/settings/toggle-override`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
