...
export function RoomsManager({ listingId }: { listingId: string }) {
  ...
  const fetchRooms = async () => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/listings/${listingId}/rooms`,
        {
          headers: {
  ...
  const handleDeleteRoom = async (roomId: string) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/rooms/${roomId}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`
...
function RoomDetails({ 
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/rooms/${room.id}`,
        {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
