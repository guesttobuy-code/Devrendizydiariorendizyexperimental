...
export function ClientSiteWrapper({ organizationId, children }: ClientSiteWrapperProps) {
  ...
  const loadSiteConfig = async () => {
    try {
      ...
      const configResponse = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/client-sites?organizationId=${organizationId}`,
        {
          headers: {
  ...
  const loadProperties = async (config: any) => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/properties?organizationId=${organizationId}&mode=${modesQuery}`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
            'Content-Type': 'application/json'
  ...
  const searchProperties = async (filters: any): Promise<Property[]> => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/properties?${queryParams}`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
            'Content-Type': 'application/json'
  ...
  const checkAvailability = async (
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/calendar/availability?propertyId=${propertyId}&startDate=${checkIn}&endDate=${checkOut}`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
            'Content-Type': 'application/json'
  ...
  const calculatePrice = async (
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/quotations/calculate`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
  ...
  const createReservation = async (reservation: Reservation) => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/reservations`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
  ...
  const sendQuotation = async (
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/quotations`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
...
export async function detectOrganizationIdByDomain(): Promise<string | null> {
  try {
    ...
    const response = await fetch(
      `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/client-sites/by-domain/${hostname}`,
      {
        headers: {
          'Authorization': `Bearer ${publicAnonKey}`,
          'Content-Type': 'application/json'
