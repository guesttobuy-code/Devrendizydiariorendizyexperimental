// ============================================================================
// MOCK BACKEND - FUNCIONA 100% NO NAVEGADOR
// ============================================================================
// Simula um backend completo usando localStorage
...
const CURRENT_DATA_VERSION = '5.1'; // CORRIGIDO - Seed data agora usa códigos curtos (RSV-XXXXXX, PRP-XXXXXX, GST-XXXXXX)
...
export function isMockEnabled(): boolean {
  ...
  // Por padrão, mock está DESATIVADO (false) - Usa backend real Supabase
  const isEnabled = value === null ? false : value === 'true';
...
export function enableMockMode() {
  ...
  console.log('🎭 Mock Backend ATIVADO - Usando localStorage');
}

export function disableMockMode() {
  localStorage.setItem(MOCK_ENABLED_KEY, 'false');
  console.log('🌐 Mock Backend DESATIVADO - Usando Supabase');
}

export function toggleMockMode() {
  const current = isMockEnabled();
  if (current) {
    disableMockMode();
  } else {
    enableMockMode();
...
// Se não existe preferência, NÃO ativa o mock (usa backend real)
// Mock só é ativado manualmente via toggle no Admin Master
...
// ============================================================================
// RESET FORÇADO AUTOMÁTICO - Detecta e corrige dados incompatíveis
// ============================================================================
if (isMockEnabled()) {
  const currentVersion = localStorage.getItem(DATA_VERSION_KEY);
  const stored = localStorage.getItem(STORAGE_KEY);
