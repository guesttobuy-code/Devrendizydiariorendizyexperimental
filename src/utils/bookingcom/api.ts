...
export class BookingComSyncManager {
  ...
  private async importReservation(reservation: BookingComReservation) {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/bookingcom/import-reservation`,
        {
          method: 'POST',
          headers: {
  ...
  private async syncPrices() {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/bookingcom/get-prices`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
          },
  ...
  private async syncAvailability() {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-67caf26a/bookingcom/get-availability`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
          },
